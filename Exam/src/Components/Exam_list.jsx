...
function Exam_list(prop) {
    const exam_list = prop.examlist.map(exam => <Link to = {`/exam/${exam.sub}/${exam.sub+exam.year.replace('/','')}`}  key={exam.sub+exam.year}><li className='flex flex-col p-4 rounded-md'><Exam_list_card sub = {exam.sub} year = {exam.year} score = '0' time = {exam.time} num = {exam.q}/></li></Link> )
  return (
    <div>
