...
function Exam_list(prop) {
  function setter(time,num){
    ...
    const exam_list = prop.examlist.map(exam => <li key={exam.sub} className='flex flex-col p-4 rounded-md'>{setter(exam.time, exam.q)}<Exam_list_card sub = {exam.sub} year = {exam.year} score = '0' time = {exam.time} num = {exam.q}/></li>)
  return (
    <div>
